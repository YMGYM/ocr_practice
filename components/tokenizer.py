...
class Tokenizer:
    def __init__(self, seq_len = 10):
        ...
        self.tokens['<SOS>'] = len(self.tokens) + 1 # start token
        self.tokens['<EOS>'] = len(self.tokens) + 1 # end token
        self.tokens['<PAD>'] = len(self.tokens) + 1 # padding token
        self.tokens['<UNK>'] = len(self.tokens) + 1 # unknown token
    ...
    def tokenize(self, input_str):
        ...
        for c in input_str:
            if self.tokens.get(c) is not None: # 사전에 글자가 있으면 토크나이징
                result.append(self.tokens[c])
            else: # 없으면 <UNK> 토큰 배정
                result.append(self.tokens['<UNK>'])
        
        result.append(self.tokens['<EOS>']) # 문장이 끝났음을 알려주는 토큰

        while(len(result) < self.seq_len): # 예측 길이를 맞출때까지 입력
            result.append(self.tokens['<PAD>'])

        # 출력 데이터의 길이는 seq_len과 같아야 한다.
        assert len(result) == self.seq_len

        one_hot_result = F.one_hot(torch.tensor(result), num_classes=len(self.tokens) + 1) # num_classes 는 0을 포함하므로 하나 더 더해주어야 한다.
